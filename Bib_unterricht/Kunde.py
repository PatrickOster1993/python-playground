...
class Kunde:
    def __init__(self, name_ein, kundennummer_ein):
        ...
        self.__ausgeliehene_buecher = []
    
    def buch_ausleihen(self, buch):
        self.__ausgeliehene_buecher.append(buch)
        print(f"{self.name} hat das Buch '{buch.titel}' ausgeliehen.")
    
    def buch_zurueckgeben(self, buch):
        if buch in self.__ausgeliehene_buecher:
            self.__ausgeliehene_buecher.remove(buch)
    ...
    def geliehende_buecher_anzeigen(self):
        if not self.__ausgeliehene_buecher:
            ...
            for buch in self.__ausgeliehene_buecher:
                print(buch.details())
